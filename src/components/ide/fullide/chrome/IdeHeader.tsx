  ...
  return (
    <div className="border-b border-neutral-200 bg-white/95 backdrop-blur dark:border-white/10 dark:bg-neutral-950/95">
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <ProjectSwitcherButton title={projectTitle} dirty={dirty} onClick={onOpenProjects} />
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" onClick={onSaveAs} disabled={saveAsDisabled} className={ACTION_BTN_CLASS}>
            Save As
          </button>
          ...
          >
            {saveBusy ? "Saving…" : canSaveCloud ? "Save Project" : hasUser ? "Upgrade to Save" : "Log in to Save"}
          </button>
        </div>
      ...
      {(showTopLanguageButtons || !isDesktop) ? (
        <div className="border-t border-neutral-200 px-2 py-2 dark:border-white/10">{languageScroller}</div>
      ) : null}
    </div>
  );
}
