      ...
      >
        ...
        {q.kind === "mcq" ? (
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2">
              ...
              />
              {(q as any).tolerance ? (
                  <div className="text-xs text-neutral-500 dark:text-white/50">± {(q as any).tolerance}</div>
              ) : null}
            </div>
        )}
        ...
        <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          ...
          <div className="text-xs font-extrabold text-neutral-600 dark:text-white/60 sm:text-right">
            {props.skipped ? (
                <span className="text-amber-700 dark:text-amber-300/80">↷ Skipped</span>
            ) : props.checked ? (
                props.ok === true ? (
                    <span className="text-emerald-700 dark:text-emerald-300/80">✓ Correct</span>
                ) : (
                    <span className="text-rose-700 dark:text-rose-300/80">✕ Not correct</span>
                )
            ) : (
                <span className="text-neutral-500 dark:text-white/50">Not checked yet</span>
            )}
          </div>
        </div>
