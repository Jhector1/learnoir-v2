...
export default function QuizBlock({
  ...
  // hydrate when quiz loads (or reload)
  useEffect(() => {
    local.hydrate(initialState ?? null);
    // practiceBank resets itself on resetKey; items hydrate from initialState inside hook
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [resetKey]);
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      allChecked,
      passed,
      allOk: questions.length > 0 && questions.every((q) => getQuestionOk(q) === true),
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [questions, local.checkedById, local.answers, practiceBank.practice, passScore]);
  ...
  }, [questions, local.answers, local.checkedById, practiceBank.practice]);

  const emitter = useDebouncedEmit(nextState, (s) => onStateChange?.(s), {
    delayMs: 400,
    enabled: Boolean(onStateChange && questions.length),
  });

  // prime dedupe after hydrate
  useEffect(() => {
    emitter.prime({
      answers: initialState?.answers ?? {},
      checkedById: initialState?.checkedById ?? {},
      practiceItemPatch: initialState?.practiceItemPatch ?? {},
      practiceMeta: initialState?.practiceMeta ?? {},
    } as SavedQuizState);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [resetKey]);
        ...
        if (q.kind === "practice") {
          ...
          return (
            <QuizPracticeCard
              ...
              onSubmit={() => {console.log("submitting practice item", q); void practiceBank.submitPractice(q);}}
          ...
          <div className="grid gap-2">
            ...
            <ul className="list-disc pl-5 space-y-1">
              ...
              <li>
                ...
                (it does <span className="font-black">not</span> generate a new set).
              </li>
            </ul>
