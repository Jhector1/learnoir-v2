...
type CommonProps<S extends { archetype: string; specVersion?: number }> = {
    spec: S;
...
type VideoLessonSpec = {
    ...
    specVersion: number;
    title?: string;
    embedUrl: string; // e.g. https://www.youtube.com/embed/xxxxx
    checkpoints?: Array<{ id: string; label: string }>;
};

export function VideoLessonSketch({ spec, value, onChange, readOnly }: CommonProps<VideoLessonSpec>) {
    const data = getData(value, { checked: {} as Record<string, boolean> });
    const checked = (data.checked ?? {}) as Record<string, boolean>;
    ...
    return (
        <div className="grid gap-3">
            <div className={SOFT2}>
                <div className="text-sm font-black text-neutral-900 dark:text-white">{spec.title ?? "Video lesson"}</div>
                <div className="mt-3 aspect-video w-full overflow-hidden rounded-xl border border-neutral-200 bg-black dark:border-white/10">
                    <iframe className="h-full w-full" src={spec.embedUrl} title={spec.title ?? "Video"} allowFullScreen />
                </div>
            </div>
                    ...
                    <div className="mt-2 grid gap-2">
                        {spec.checkpoints.map((c) => (
                            <label key={c.id} className="flex items-start gap-2 text-sm">
                                <input type="checkbox" checked={Boolean(checked[c.id])} onChange={() => toggle(c.id)} disabled={readOnly} />
                                <span className="text-neutral-800 dark:text-white/80">{c.label}</span>
                            </label>
                        ))}
                    </div>
                </div>
